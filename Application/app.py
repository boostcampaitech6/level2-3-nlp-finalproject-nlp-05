...
async def generate_poem(request: PoemRequest):
    ...
    image_prompt = f"create a watercolor scene for the following poem. Consider the factors to reinforce the feelings that fit the poem." + '\n' + 'poem:' + '\n' + poem + '\n'
    
    # 이미지 생성
    # OpenAI API_KEY 설정
    API_KEY = tokens.openai.api_key_sanggi
